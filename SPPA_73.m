function [T,V,Var,kurt,pop,fitness]=SPPA_73(X,varargin)
%PPGA7 Variable selection for sparse projection pursuit analysis, with a genetic
%algorithm (version 7.3). This version allows the choice between dimension
%by dimension separation, or separation based on the sum or product of
%kurtoses.
%
%Outputs:
%
%T = PPGA7(X) performs parse projection pursuit on the m*n (samples x variables)
%       matrix X, and returns the scores in T, a m*dim matrix, where dim is
%       the number of dimensions of separation.
%
%[T,V] = PPGA7(X) also returns the vectors in V, a dim*n matrix containing the
%       projection vectors for each dimension.
%
%[T,V,VAR] = PPGA7(X) returns the variables in VAR, a  dim*nvars matrix, with each
%       column containing the chosen variables for each dimension of separation.
%
%[T,V,VAR,KURT] = PPGA7(X) also returns the kurtosis value(s) for the
%       solution, in a dim*1 matrix, with each dimension having a kurtosis
%       value associated with the separation.
%Inputs:
%
%PPGA7(X) returns a plot without colour-coded classes
%PPGA7(X,objclass) returns a plot that is colour-coded according to "objclass",
%       a 1*m cell array or matrix containing the class number for each sample.
%PPGA7(X,objclass,classlist) returns a colour-coded plot and legend
%       based on "classlist", a 1*'number of classes' cell array containing labels
%       for each class.
%PPGA7(X,...,Optionname,Optionvalue,...) performs parse projection
...
%           it hasn't converged
%           Default: 300
%    Sumprod: Option to optimize with respect to the sum (sum) or product
%    (prod) of the kurtoses in each dimension. If not, the algorithm
%    optimizes iteratively on each dimension (norm).
%          Default: norm
...
%
%
%Example: To perform projection pursuit with a colour-coded plot according to 'class',
%10 variables, using recentered kurtosis and a mutation rate of 20%, enter:
%   [T,V,VAR] = PPGA7(X,class,'nvars',10,'opt','rec','mutrate',0.2)
...
%
    ...
    if strcmp('mul',f.meth)==true
        [scores,vectors,PPOUT]=projpursuit(X(:,variables(d,:)),pursuitdim,'mul');
        kurt=PPOUT.K;
        pops=pop;
        break
    end
